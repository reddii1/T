...
echo "Restoring $FILE_PATH from branch: $BRANCH..."
...
if [ "$status_code" -eq 200 ]; then
  ...
  echo "Successfully fetched $FILE_PATH from branch: $BRANCH. Writing to $CSV_DIR/$FILE_PATH..."
  ...
  echo "Error: Failed to fetch $FILE_PATH from branch: $BRANCH. HTTP Status: $status_code"
  echo "Response: $file_content"
  exit 1
fi

echo "Restore of $FILE_PATH from branch: $BRANCH completed successfully."